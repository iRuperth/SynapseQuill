...
def narrate(match: Match, *, language: str = "es", system_preamble: str = "",
    ...
    system = (system_preamble + "\n\n" if system_preamble else "") + (
        ...
        "- Announce minutes in a NATURAL, human, broadcaster way — NEVER the robotic "
        "'minuto 34, amarilla para X'. Vary the phrasing: 'al minuto 34 llega la primera "
        "tarjeta amarilla, para X', 'hacia la media hora veíamos la roja de Y', 'ya en el 60, "
        "amonestado Z'. For the FIRST card of the match, say so explicitly ('la primera "
        "amarilla del partido'). Mix these forms so it never sounds like a list.\n"
